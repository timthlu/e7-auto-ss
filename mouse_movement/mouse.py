...
def mouse_bez(init_pos, fin_pos, deviation, speed):
    ...
    control_1 = (init_pos[0] + choice((-1, 1)) * abs(ceil(fin_pos[0]) - ceil(init_pos[0])) * (1 / time_stamps) * randint(deviation // 2, deviation),
                init_pos[1] + choice((-1, 1)) * abs(ceil(fin_pos[1]) - ceil(init_pos[1])) * (1 / time_stamps) * randint(deviation // 2, deviation)
                    )
    control_2 = (init_pos[0] + choice((-1, 1)) * abs(ceil(fin_pos[0]) - ceil(init_pos[0])) * (1 / time_stamps) * randint(deviation // 2, deviation),
                init_pos[1] + choice((-1, 1)) * abs(ceil(fin_pos[1]) - ceil(init_pos[1])) * (1 / time_stamps) * randint(deviation // 2, deviation)
                    )
